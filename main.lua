...
ReferenceButton.Position = UDim2.new(0, 0, 0, 2)
...
CMDs[#CMDs + 1] = {NAME = 'randomserver / rs', DESC = 'Makes you join a random server'}
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/Inteli914900ks/moddediy/refs/heads/main/main.lua'))()")
	end
end)
...
addcmd("randomserver", {"rs"}, function(args, speaker)
	local servers, cursor = {}, ""

	repeat
		local url = ("https://games.roblox.com/v1/games/%d/servers/Public?limit=100&sortOrder=Desc&cursor=%s"):format(PlaceId, cursor or "")
		...
		for _, s in ipairs(res.data) do
			if s.playing and s.maxPlayers and s.playing < s.maxPlayers and tostring(s.id) ~= tostring(JobId) then
	...
	if #servers > 0 then
		TeleportService:TeleportToPlaceInstance(PlaceId, servers[math.random(#servers)], Players.LocalPlayer)
	else
		return notify("No available servers to hop into.")
	end
end)
